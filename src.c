...
long fetch() { //fetch an instruction from a instruction memory
	// IF
	// inst_mem saves 32bit data per index,
	// the unit of movement of pc is 8 bit per 1.
	// so in the simulation, we should divide the PC value into 4,
	// so that we can read inst_mem by indexing.
...
int main(int ac, char* av[])
{
	...
	while (!done)
	{
		regs[0] = 0; // x0 is hard-wired into 0
		instruction = fetch();
